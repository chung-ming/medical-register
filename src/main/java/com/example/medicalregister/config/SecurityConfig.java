...
public class SecurityConfig {
        ...
        public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
                ...
                http
                                .authorizeHttpRequests(authorizeRequests -> {
                                        authorizeRequests
                                                        .requestMatchers("/", "/error", "/public/**", "/test/login",
                                                                        "/api/v1", // Permit the root API endpoint
                                                                        "/h2-console/**" // Allow access to H2 console
                                                                                         // (intended for development)
                                        ).permitAll()
                                                        // Allow unauthenticated access to the health endpoint
                                                        .requestMatchers(EndpointRequest.to("health")).permitAll()
                                                        // Permit requests dispatched via FORWARD (e.g., to error pages)
                                                        // or ERROR.
                                                        // This is crucial for Spring Boot's default error handling to
                                                        // function
                                                        ...
                                                        .requestMatchers("/api/v1/**").authenticated() // This will
                                                                                                       // still secure
                                                                                                       // /api/v1/records
                                                                                                       // etc.
                                ...
                                // For APIs, session management can often be stateless if using tokens (e.g.,
                                // JWTs from client)
                                // .sessionManagement(session ->
                                // session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                                .oauth2Login(oauth2Login -> oauth2Login
                                                .defaultSuccessUrl("/records", true) // Always redirect to records list
                                                                                     // after login
                                )
                                .logout(logout -> logout
                                                .logoutRequestMatcher(new AntPathRequestMatcher("/logout")) // Spring
                                                                                                            // Security
                                                                                                            // handles
                                                                                                            // this URL
                                                .addLogoutHandler(logoutHandler()) // Custom Auth0 logout handler for
                                                                                   // RP-initiated logout
                                )
                                ...
                                .csrf(csrf -> csrf
                                                ...
                                                .ignoringRequestMatchers("/api/v1/**") // Typically disable CSRF for
                                                                                       // stateless APIs
                                )
                ...
                return (request, response, authentication) -> {
                        try {
                                ...
                                String baseLogoutUrl = UriComponentsBuilder.fromUriString(issuerUri)
                                                .path("/v2/logout") // .path() handles leading/trailing slashes
                                                                    // correctly
                                                .build()
                                                .toUriString();
