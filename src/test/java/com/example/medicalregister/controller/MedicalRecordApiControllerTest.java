...
class MedicalRecordApiControllerTest {
    ...
    void setUp() {
        ...
        sampleRecord1 = new MedicalRecord(1L, "Patient Zero", 30, "Initial History", TEST_USER_SUB_VALUE,
                TEST_USER_SUB_VALUE,
                TEST_USER_SUB_VALUE, LocalDateTime.now(), LocalDateTime.now());
    ...
    void createRecord_validData_shouldCreateRecord() throws Exception {
        MedicalRecord newRecordInput = new MedicalRecord(null, "New Patient", 25, "New History", null, null, null, null,
                null);
        ...
        mockMvc.perform(post("/api/v1/records")
                ...
                .andExpect(header().string("Location", endsWith("/api/v1/records/" + savedRecord.getId())))
    ...
    void getRecordById_recordFound_shouldReturnRecord() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/records/{id}", sampleRecord1.getId()).with(oauth2Login().oauth2User(testUser)))
                .andExpect(status().isOk())
    ...
    void getRecordById_recordNotFound_shouldReturnNotFound() throws Exception {
        when(medicalRecordService.findRecordById(anyLong())).thenThrow(new RecordNotFoundException("Record not found"));
    ...
    void updateRecord_validData_shouldUpdateRecord() throws Exception {
        MedicalRecord updatedDetails = new MedicalRecord(sampleRecord1.getId(), "Updated Name", 32, "Updated History",
    ...
    void updateRecord_accessDenied_shouldReturnForbidden() throws Exception {
        MedicalRecord updateData = new MedicalRecord(1L, "Attempted Update", 40, "History", null, null, null, null,
                null);
        when(medicalRecordService.saveRecord(any(MedicalRecord.class)))
                .thenThrow(new AccessDeniedException("Permission denied to update"));
        ...
        mockMvc.perform(put("/api/v1/records/{id}", 1L)
                ...
                .andExpect(jsonPath("$.message", is("Permission denied to update")));
    }
    ...
    void deleteRecord_recordNotFound_shouldReturnNotFound() throws Exception {
        doThrow(new RecordNotFoundException("Record to delete not found")).when(medicalRecordService)
                .deleteRecordById(anyLong());

        mockMvc.perform(delete("/api/v1/records/{id}", 99L)
                .with(oauth2Login().oauth2User(testUser)))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.message", is("Record to delete not found")));
    }
}
